...
- Python 3.10+
- Codex CLI authenticated with a ChatGPT/Codex subscription
...
By default, the example calls `codex exec` through the local Codex CLI, so it
uses your existing ChatGPT/Codex subscription login rather than an OpenAI API
key. You can optionally set `CODEX_MODEL`, `CODEX_PROFILE`, `CODEX_BINARY`,
`CODEX_TIMEOUT_SECONDS`, or `CODEX_EXTRA_ARGS` to control the CLI invocation.
